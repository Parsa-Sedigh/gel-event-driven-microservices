...
/* @ConditionalOnExpression means load this class only if twitter-to-kafka-service.enable-v2-tweets configuration variable is true and
twitter-to-kafka-service.enable-mock-tweets is false.
...
public class TwitterV2StreamHelper {
    ...
    private List<String> getRules(String bearerToken) throws URISyntaxException, IOException {
        ...
        if (null != entity) {
            ...
            // with json.has("data"), when we call the api for the first time and when there is no rule, we won't get a parse error.
